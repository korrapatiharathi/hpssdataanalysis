...
my $stmt = qq( select username, count(*) as total_commands from gridftp.hpss_2016 where (start_time BETWEEN to_date('$bucket_start_date','YYYY-MM-DD') AND to_date('$bucket_end_date','YYYY-MM-DD')) AND (end_time BETWEEN to_date('$bucket_start_date','YYYY-MM-DD') AND to_date('$bucket_end_date','YYYY-MM-DD')) group by username order by total_commands desc limit 10;);
...
print $fh "User_Name,total_commands";

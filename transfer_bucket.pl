...
my $stmt = qq( select username , count(*) as No_File_transfers, cast(sum(bytes) as decimal)/cast(sum(elapsed) as decimal) as avg_transfer_rate from (select username,file,bytes,elapsed from gridftp.hpss_2016 where (start_time BETWEEN to_date('$bucket_start_date','YYYY-MM-DD') AND to_date('$bucket_end_date','YYYY-MM-DD')) AND (end_time BETWEEN to_date('$bucket_start_date','YYYY-MM-DD') AND to_date('$bucket_end_date','YYYY-MM-DD'))) as foo group by username;);
...
print $fh "User_Name,File_Count, Transfer_rate";
